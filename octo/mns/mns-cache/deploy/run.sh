...
JVM_GC=$JVM_GC" -XX:CMSFullGCsBeforeCompaction=0 -XX:+UseCMSCompactAtFullCollection -XX:CMSInitiatingOccupancyFraction=75"
...
function run() {
    ...
    EXEC_JAVA=$EXEC_JAVA" -Djetty.appkey=$APP_KEY -Djetty.context=$CONTEXT -Djetty.logs=$LOG_PATH"
    ...
    echo "$(date) mnsc deploy/run.sh " >> /opt/logs/mobile/mns-cache/run.log
