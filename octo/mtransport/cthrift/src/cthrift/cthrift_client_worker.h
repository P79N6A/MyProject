 ...
 public:
  ...
  ~ConnInfo(void) {
    if (CTHRIFT_UNLIKELY(!p_map_weight_tcpclientwp_)) {
      CLOG_STR_ERROR("p_map_weight_tcpclientwp_ NULL");
    } else {
      CLOG_STR_INFO("delete appkey " << sgservice_.appkey << " ip: "
...
class CthriftClientWorker {
 private:
  struct CnxtEntry: public muduo::copyable {
      ...
      explicit CnxtEntry(const WeakContSharedPtr &wp_cnxt_,
                         CthriftClientWorker *parent)
              : wp_cnxt_(wp_cnxt_), p_worker_(parent) {}
  ...
  boost::unordered_map <std::string, SGService>
  ...
  void HandleUniformMsg(const muduo::net::TcpConnectionPtr &conn, const int32_t &length, uint8_t *buf) ;

  void HandleHessianMsg(const muduo::net::TcpConnectionPtr &conn) ;

  void HandleThriftMsg(const muduo::net::TcpConnectionPtr &conn, const std::string& ret_msg, const int32_t &length, uint8_t *buf) ;
  ...
  void UpdateAuthToken(const std::string&  token);

  void InitWorker(void);
  void InitSgagentHandlerThread(void);
  void InitAuthTokenUpdateThread(void);
  ...
  void GetAuthToken(void);

  int8_t ChooseNextReadyConn(TcpClientWeakPtr *p_wp_tcpcli);
  static int8_t CheckRegion(const double &d_weight);

  cthrift_protocol_type DoPreOperationWhenSend(const std::string& seq_id, muduo::net::Buffer& buffer, SGService& sgservice,
                                               const std::string& traceid, const std::string& spanid);

  //filter
  bool FilterAll(const SGService& sg);
  bool FilterService(const SGService& sg);
  bool FilterPort(const SGService& sg);
 ...
 public:
  ...
  void ClearTcpClient(muduo::CountDownLatch *p_clear_countdown) {
    CLOG_STR_DEBUG("into worker thread clean resource");
  ...
  void ReportTimeToCat(const bool& success, const double& time);

  void DelContextMapByID(const std::string &str_id) {

    //cat upload timeout info
    MapID2SharedPointerIter
            map_iter = map_id_sharedcontextsp_.find(str_id);
    if(map_iter != map_id_sharedcontextsp_.end()) {

      ReportTimeToCat(false,  timeDifference(Timestamp::now(), map_iter->second->timestamp_start) * 1000);

      boost::shared_ptr <muduo::net::TcpConnection> sp_send_conn((map_iter->second->wp_send_conn).lock());
      if(sp_send_conn) {
        CLOG_STR_WARN("del id " << str_id << " from ipport :" << (sp_send_conn->peerAddress()).toIpPort()
                                << " upper_spanid " << map_iter->second->str_upper_spanid_
                                << " upper_traceid " << map_iter->second->str_upper_traceid_);
      }
    }

    CLOG_STR_WARN("del id " << str_id << " by transport for timeout");
  ...
  void AsyncBadCallback(SharedContSharedPtr sp_shared);
  ...
  std::string GetServerAppkey(){
    return str_svr_appkey_;
  }
};
}
