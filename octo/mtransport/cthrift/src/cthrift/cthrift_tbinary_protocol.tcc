//
// Created by Chao Shu on 16/12/18.
//

#ifndef CTHRIFT_CTHRIFT_TBINARY_PROTOCOL_TCC
#define CTHRIFT_CTHRIFT_TBINARY_PROTOCOL_TCC
...
int32_t CthriftTBinaryProtocolT<TMemoryBuffer>::GetSeqID(void) {
  ...
  if (0 > i32_size && ((i32_size & VERSION_MASK) == VERSION_1)) {
    ...
    CLOG_STR_DEBUG("seq_id " << i32_seq_id);
    return i32_seq_id;
...
writeMessageBegin(const std::string &name,
                  ...
                  const int32_t seqid) {
  CLOG_STR_DEBUG("async writeMessageBegin");
  ...
  if (0 == seqid) {
    ...
    if(CTHRIFT_UNLIKELY(std::numeric_limits<int32_t>::max() == i32_seq_id)){
      CLOG_STR_WARN("i32_seq_id " << i32_seq_id << " up to int32 max value, reset to 1");
...
writeMessageBegin(const std::string &name,
  ...
  if (CTHRIFT_CLIENT == use_type_ && 0 == seqid) {
    ...
    boost::dynamic_pointer_cast<CthriftTransport>(getTransport())->ResetWriteBuf();
    CLOG_STR_DEBUG("clear write buf done");

    boost::dynamic_pointer_cast<CthriftTransport>(getTransport())->SetID2Transport(
...
CthriftTBinaryProtocolT<Transport_>::writeMessageBegin(const std::string &name,
                                                       const TMessageType messageType,
                                                       const int32_t seqid) {
  CLOG_STR_DEBUG("Transport_ writeMessageBegin");
  if (this->strict_write_) {
    int32_t version = (VERSION_1) | ((int32_t) messageType);
    uint32_t wsize = 0;
    wsize += writeI32(version);
...
uint32_t CthriftTBinaryProtocolT<Transport_>::readStringBody(std::string &str,
  ...
  if (this->string_limit_ > 0 && size > this->string_limit_) {
    CLOG_STR_ERROR("throw  TProtocolException::SIZE_LIMIT : " << this->string_limit_
              << " read size:" << size);
    throw TProtocolException(TProtocolException::SIZE_LIMIT);
  }
