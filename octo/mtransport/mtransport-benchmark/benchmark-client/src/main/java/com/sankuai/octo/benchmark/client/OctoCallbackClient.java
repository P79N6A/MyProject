 ...
 * Time: 下午5:46
 */
public class OctoCallbackClient {

    private final static Logger logger = LoggerFactory.getLogger(OctoCallbackClient.class);
    ...
    public OctoCallbackClient(int threadNum, long msgCount, int msgLength, String msgType) {
    ...
    public void test() {
        ...
        logger.info("*****OctoCallback:ThreadNum:" + threadNum + " MsgCount:" + msgCount + " MsgLength:" + msgLength + " MsgType:" + msgType + "*****");
            ...
            try {
                while (true) {
                    ...
                    if ("pojo".equals(msgType)) {
                        client.sendPojo(msgList, callback);
                    } else if ("string".equals(msgType)) {
                        client.sendString(str, callback);
                    } else if ("byte".equals(msgType)) {
                        client.sendBytes(buffer, callback);
                    }
