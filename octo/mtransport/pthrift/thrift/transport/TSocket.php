...
class TSocket extends TTransport {
  ...
  public function readAll($len) {
    ...
    // check if memory is enough, added by weiziqian
    $memleft = $this->getMemoryLeft();
    if (!is_null($memleft) && $memleft < $len) {
        throw new TException('TSocket: will dry up memory while reading '.$len.' bytes from '.
                               $this->host_.':'.$this->port_);
    }

    $pre = null;
    if ($len > 1000*1000*1000) {
        $error = var_export(debug_backtrace(), true);
   ...
   * added by weiziqian
   */
  private function getMemoryLeft()
  {
    ...
    if (empty($memoryLimit)) {
        ...
        $units = strtolower($val[strlen($val)-1]);
        $val = intval($val);
        switch($units) {
          case 'g':
            $val *= 1024;
    ...
    return ($memoryLimit < $used) ? null : $memoryLimit - $used;
  }
}
