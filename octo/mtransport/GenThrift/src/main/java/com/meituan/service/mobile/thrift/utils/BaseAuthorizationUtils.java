...
/**
 * 
 * 主站BA验证相关Utils,http://wiki.sankuai.com/pages/viewpage.action?pageId=29755412
 * 
 * @author guanfei
 * @created 2012-7-18
 * 
...
public class BaseAuthorizationUtils {
    ...
    private BaseAuthorizationUtils(){}
     ...
     * 
     */
    public static void generateAuthAndDateHeader(HttpRequestBase request,
            String client, String secret) {
        Date sysdate = new Date();
        SimpleDateFormat df = new SimpleDateFormat(HTTP_HEADER_DATE_FORMAT,
                Locale.US);
        df.setTimeZone(TimeZone.getTimeZone(HTTP_HEADER_TIME_ZONE));
        String date = df.format(sysdate);
        String string_to_sign = request.getMethod().toUpperCase() + " "
                + request.getURI().getPath() + "\n" + date;
        String sig = secret;
        String encoding = "";
        try {
            byte[] bb = getSignature(string_to_sign.getBytes(), sig.getBytes());
            encoding = new String(Base64.encodeBase64(bb));
        } catch (Exception e1) {
            return;
        }
        String authorization = MEITUAN_AUTH_METHOD + " " + client + ":"
                + encoding;
