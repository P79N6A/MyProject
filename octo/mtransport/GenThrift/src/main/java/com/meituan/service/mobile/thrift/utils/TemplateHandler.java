...
public class TemplateHandler {
    ...
    public String genBusiServer(String pack) {
        String template = readFile("/BusinessFrame/Server/Server.template");
        template = template.replace("${PACKAGE}", pack);
        return template;
    }

    public String genBusiServiceImpl(String pack, String service, List<String> methodList) {

        String template = readFile("/BusinessFrame/Server/ServiceImpl.template");
    ...
    public String genBusiServerXML(String servicePath) {

        String template = readFile("/BusinessFrame/Server/ServerXML.template");
        template = template.replace("${SERVICEPATH}", servicePath);
        return template;
    }

    public String genBusiPom(String projectName) {

        String template = readFile("/BusinessFrame/pom.template");
        template = template.replace("${PROJECTNAME}", projectName);
        return template;
    }

    public String genBusiClient(String pack, String service) {

        String template = readFile("/BusinessFrame/Client/Client.template");
        template = template.replace("${PACKAGE}", pack);
        template = template.replace("${SERVICE}", service);
        return template;
    }

    public String genBusiClientXML(String pack, String servicePath) {

        String template = readFile("/BusinessFrame/Client/ClientXML.template");
        template = template.replace("${PACKAGE}", pack);
        template = template.replace("${SERVICEPATH}", servicePath);
        return template;
    }
