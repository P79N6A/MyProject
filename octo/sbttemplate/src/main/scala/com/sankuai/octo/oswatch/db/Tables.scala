...
trait Tables {
  ...
  implicit def GetResultOswatchMonitorPolicyRow(implicit e0: GR[Long], e1: GR[String], e2: GR[Option[String]], e3: GR[Int], e4: GR[Double]): GR[OswatchMonitorPolicyRow] = GR{
    prs => import prs._
    OswatchMonitorPolicyRow.tupled((<<[Long], <<[String], <<?[String], <<[Int], <<[Int], <<[Int], <<[Int], <<[Double], <<?[String], <<[String]))
  }
  ...
  class OswatchMonitorPolicy(_tableTag: Tag) extends Table[OswatchMonitorPolicyRow](_tableTag, "oswatch_monitor_policy") {
    def * = (id, appkey, idc, env, gtetype, watchperiod, monitorType, monitorvalue, spanName, responseurl) <> (OswatchMonitorPolicyRow.tupled, OswatchMonitorPolicyRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (id.?, appkey.?, idc, env.?, gtetype.?, watchperiod.?, monitorType.?, monitorvalue.?, spanName, responseurl.?).shaped.<>({r=>import r._; _1.map(_=> OswatchMonitorPolicyRow.tupled((_1.get, _2.get, _3, _4.get, _5.get, _6.get, _7.get, _8.get, _9, _10.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
