...
JVM_GC="-XX:+PrintGCDetails -XX:+PrintFlagsFinal -XX:+PrintHeapAtGC -XX:+PrintTenuringDistribution -XX:+UseConcMarkSweepGC -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps"
JVM_GC=$JVM_GC" -XX:CMSFullGCsBeforeCompaction=0 -XX:+UseCMSInitiatingOccupancyOnly -XX:+UseCMSCompactAtFullCollection -XX:CMSInitiatingOccupancyFraction=80"
...
# ------------------------------------
MODULE=msgp-server
WORK_PATH=/opt/meituan/apps/msgp-server
LOG_PATH=/opt/logs/apps/msgp-server
...
function run() {
	...
	$EXEC_JAVA com.sankuai.mms.boot.Bootstrap > /tmp/msgp.start.log 2>&1
}
