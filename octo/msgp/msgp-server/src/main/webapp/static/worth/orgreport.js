...
var _tableTd = "owt";
var tbody,pbody;
M.add('msgp-worth/orgreport', function (Y) {
    Y.namespace('msgp.worth').orgreport = orgreport;
    function orgreport(report_style,business) {
        _report_style = report_style;
        Y.one('#business').set('value', business);
        tbody = Y.one('#table_wrapper tbody');
        pbody = Y.one('#paginator_wrapper');
        Y.one('#label_text').append(Y.one('#text_'+report_style).get("value"));
        initDatePicker();
        initTable()
        bindSearch()
    }
    ...
    function initTable() {
        searchData['business'] = Y.Lang.trim(Y.one('#business option:checked').get('value'));
        if (_report_style == "org_owt") {
            searchData['owt'] = Y.Lang.trim(Y.one('#owt').get('value'));
            _tableTd = "owt";
        } else if (_report_style == "org_user") {
            searchData['username'] = Y.Lang.trim(Y.one('#username').get('value'));
            _tableTd = "username";
        } else if (_report_style == "org_model") {
    ...
    function fillTableContent(data) {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            d = data[i];
            html += ['<tr>',
                '<td><span class="t-ellipsis" >' + d.business + '</span></td>',
                '<td><span class="t-ellipsis" >' + d[_tableTd] + '</span></td>',
