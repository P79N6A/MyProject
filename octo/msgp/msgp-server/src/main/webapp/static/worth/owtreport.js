var searchData = {
    pageNo: 1,
    pageSize: 20
}, _report_style = "owt", colspan = 4;
var everPaged = false,
    totalPage= 0,
    totalCount= 0;
var _tableTd = "owt";
var tbody,pbody;
M.add('msgp-worth/owtreport', function (Y) {
    Y.namespace('msgp.worth').owtreport = owtreport;
    function owtreport(report_style) {
    ...
    function initTable() {
        ...
        if (_report_style == "owt") {
            searchData['appkey'] = Y.Lang.trim(Y.one('#appkey').get('value'));
            _tableTd = "appkey";
        }else if (_report_style == "owt_model") {
            searchData['model'] = Y.Lang.trim(Y.one('#model').get('value'));
            _tableTd = "model";
        }
    ...
    function fillTableContent(data) {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            d = data[i];
            html += ['<tr>',
                '<td><span class="t-ellipsis" >' + d.owt + '</span></td>',
