M.add('msgp-servicerep/topqpstp', function(Y){
    Y.namespace('msgp.servicerep').topqpstp = topqpstp;
    var tbody = Y.one('#topqpstp_wrap tbody'),
        colspan = 9;

    function topqpstp(owt, day) {
        getHead(day);
        initTable(owt, day)
    }
    ...
    function getHead(day) {
        ...
        html += '<th>';
        Y.one('#topqpstp_wrap thead').setHTML(html)
    }
    ...
    function fillTableContent(data) {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            ...
            tdDatas.push(appkeyDatas[0].intro)
