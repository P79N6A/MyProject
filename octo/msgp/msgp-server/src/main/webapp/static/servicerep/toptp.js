M.add('msgp-servicerep/toptp', function(Y){
    Y.namespace('msgp.servicerep').toptp = toptp;
    var tbody = Y.one('#toptp_wrap tbody'),
        colspan = 9;
    function toptp(owt, day){
        getHead(day);
        initTable(owt, day)
    }
    function getHead(day) {
        ...
        html += '</th>';
        Y.one('#toptp_wrap thead').setHTML(html)
    }
    ...
    function fillTableContent(data) {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            ...
            var tdDatas = Array();
            tdDatas.push("<a  target='_blank' href='/data/dashboard?appkey=" +appkey +"'>" +appkey +"</a>");
