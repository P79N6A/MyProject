(function (window) {
    ...
    jQuery.get(getRelativePath("../../static/hystrixCommand/templates/hystrixCircuit.html"), function (data) {
        hystrixTemplateCircuit = data;
    });
    jQuery.get(getRelativePath("../../static/hystrixCommand/templates/hystrixCircuitContainer.html"), function (data) {
    ...
    window.HystrixCommandMonitor = function (index, containerId, args) {
        ...
        var maxRadiusForCircle = "100";

        // CIRCUIT_BREAKER circle visualization settings
        self.circuitCircleRadius = d3.scale.pow().exponent(0.5).domain([0, 300]).range(["5", maxRadiusForCircle]); // requests per second per host
        self.circuitCircleYaxis = d3.scale.linear().domain([0, 300]).range(["30%", maxXaxisForCircle]);
        self.circuitCircleXaxis = d3.scale.linear().domain([0, 300]).range(["30%", maxYaxisForCircle]);
        self.circuitColorRange = d3.scale.linear().domain([1, 25, 40, 50]).range(["grey", "#FFCC00", "#FF9900", "red"]);
        self.circuitErrorPercentageColorRange = d3.scale.linear().domain([1, 10, 35, 50]).range(["grey", "black", "#FF9900", "red"]);
        ...
        function displayCircuit(data) {
            ...
            var ratePerSecondPerHostDisplay = ratePerSecond;
            // set the rates on the div element so it's available for sorting
            var errorThenVolume =   (data.errorCount * 100000000) +  data.rollingCountSuccess;
            //console.log(data.desc+","+data.name+","+errorThenVolume)
            $('#CIRCUIT_' + data.escapedName).attr('success_value', data.rollingCountSuccess);
            $('#CIRCUIT_' + data.escapedName).attr('error_value', errorThenVolume);

            // update errorPercentage color on page
            $('#CIRCUIT_' + data.escapedName + ' a.errorPercentage').css('color', self.circuitErrorPercentageColorRange(data.errorCount));

            updateCircle('circuit', '#CIRCUIT_' + data.escapedName + ' circle', ratePerSecondPerHostDisplay, data.errorCount);

            if (data.graphValues) {
                // we have a set of values to initialize with
                updateSparkline('circuit', '#CIRCUIT_' + data.escapedName + ' path', data.graphValues, currentTime);
            } else {
                updateSparkline('circuit', '#CIRCUIT_' + data.escapedName + ' path', ratePerSecond, currentTime);
            }
        ...
        /* private */
        function updateCircle(variablePrefix, cssTarget, rate, errorCount) {
            ...
            d3.selectAll(cssTarget)
                ...
                .style("fill", self[variablePrefix + 'ColorRange'](errorCount));
        }

        /* private */
        function updateSparkline(variablePrefix, cssTarget, newDataPoint, currentTime) {
            var currentTimeMilliseconds = currentTime ? currentTime : new Date().getTime();
            ...
            while (data.length > 100) { // 400 should be plenty for the 2 minutes we have the scale set to below even with a very low update latency
            ...
            var xScale = d3.time.scale().domain([new Date(currentTimeMilliseconds - (60 * 1000*5 )), new Date(currentTimeMilliseconds)]).range([0, 120]);
    ...
    HystrixCommandMonitor.prototype.sortByVolumeInDirection = function (direction) {
        this.sortedBy = 'success_' + direction;
        $('#' + this.containerId + ' div.monitor').tsort({order: direction, attr: 'success_value'});
    ...
    HystrixCommandMonitor.prototype.sortByErrorInDirection = function (direction) {
        ...
        $('#' + this.containerId + ' div.monitor').tsort({order: direction,attr:'error_value'});
    };
    ...
    HystrixCommandMonitor.prototype.sortByLatency50 = function () {
        var direction = "desc";
        if (this.sortedBy == 'lat50_desc') {
            direction = 'asc';
        }
        this.sortedBy = 'lat50_' + direction;
        this.sortByMetricInDirection(direction, ".latency50 .value");
    };
    ...
    HystrixCommandMonitor.prototype.sortSameAsLast = function () {
        ...
        } else if (this.sortedBy == 'lat50_asc') {
            this.sortByMetricInDirection('asc', '.latency50 .value');
        } else if (this.sortedBy == 'lat50_desc') {
            this.sortByMetricInDirection('desc', '.latency50 .value');
