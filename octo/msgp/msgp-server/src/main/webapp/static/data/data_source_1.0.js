M.add('msgp-data/data_source_1.0', function (Y) {
    Y.namespace('msgp.data').data_source = data_source;
    var appkey;
    var startInput, endInput;
    var sdate, edate;
    var sourceWrapper, listWrapper;
    var lastTitleKey, chartsData;
    ...
    function data_source(_appkey, _isFirstIn) {
        appkey = _appkey;
        document.title = "来源分析";
        if (!_isFirstIn) {
            return;
    ...
    function initParams() {
        sourceWrapper = Y.one('#div_data_source');
        startInput = sourceWrapper.one('#start_time');
        endInput = sourceWrapper.one('#end_time');
        listWrapper = sourceWrapper.one('.source_kpi_list');
        compareWrap = sourceWrapper.one('#compare_wrap');
        Y.msgp.service.setEnvText('source_env_select');

        /* sourceWrapper.one('#remoteApp').set('value', '*');
         sourceWrapper.one('#spanname').set('value', 'all');*/
    }
    ...
    function initAppkeyTags() {
        ...
        var env = sourceWrapper.one("#source_env_select a.btn-primary").getAttribute('value');
        var requestPara = {
            appkey: appkey,
            env: env,
            source: 'server'
    ...
    function initEvent() {
        sourceWrapper.one('#query_btn').delegate('click', function () {
            if (searching) return;
            sourceWrapper.all("#query_btn .btn").removeClass("btn-primary");
            this.addClass("btn-primary");
            this.hasClass("chart") ? getTableData(true, listWrapper, {}) : getGraphsData();
        }, ".btn");

        sourceWrapper.one("#source_env_select").delegate('click', function () {
            var el = this;
            if (el.hasClass('btn-primary')) return;
            sourceWrapper.all('#source_env_select a').removeClass('btn-primary');
            this.addClass("btn-primary");
            initAppkeyTags();
        }, "a");
        ...
        listWrapper.delegate('click', function () {
            ...
            if (!previous || (now - previous > 300)) {
                ...
                if ($("." + _id).length) {
                    ...
                    var wrapper = sourceWrapper.one("#charts_outer");
                    ...
                    getTableData(false, wrapper, params);
                }
                previous = now;
            }
        }, '.day_graph_click');
        ...
        sourceWrapper.one('#compare_button').on('click', function () {
            lastLegend = [];
            compareData = [];
        ...
        sourceWrapper.one("#query_btn .btn-primary").hasClass("chart") ? getTableData(true, listWrapper, {}) : getGraphsData();
    }

    function initList(data) {
        var micro = new Y.Template();
        var str = micro.render(tableTemplate(), {data: data});
        sourceWrapper.one(".source_kpi_list").setHTML(str);
        Y.mt.widget.init();
    }
    ...
    function showHostDetail(data, appkey, totalCount) {
        if (data.length == 0) {
            ...
            Y.Array.each(data, function (item, index) {
                ...
                detailData.push({
                    localhost: item.remoteHostDesc.name,
                    count: item.count,
                    ...
                    http5xxCount: failureDetails.http5XXCount
                })
            });
    ...
    function getTableData(merge, node, params) {
        var se = getStartEnd();
        if (!se) return;
        var env = sourceWrapper.one("#source_env_select a.btn-primary").getAttribute('value');
        var queryBtn = sourceWrapper.all("#query_btn .btn-primary");
        if (!params.remoteApp) {
            var remoteApp = sourceWrapper.one("#remoteApp").get("value");
            var spanname = sourceWrapper.one("#spanname").get("value");
    ...
    function getHostDetailData(remoteApp, spanname, count) {
        ...
        var se = getStartEnd();
        if (!se) return;
        var dialogTitle = '服务' + remoteApp + '调用详情';
    ...
    function fillCharts() {
        ...
        if (wrapper) {
            ...
            insertChartsWrapper(2, wrapper);
        }
        // delay charts init
        setTimeout(function () {
            var nds = sourceWrapper.all('.charts-wrapper-out');
    ...
    function insertChartsWrapper(n, wrapper) {
        var node;
        for (var i = 0; i < n; i++) {
    ...
    function wrapOption(obj) {
        ...
        var legend = {
            bottom: 0,
            data: [{name: '50%耗时', icon: 'line'}, {name: '90%耗时', icon: 'line'}, {name: '99%耗时', icon: 'line'}]
        };

        if (typeof(obj.dropQps) != "undefined" && obj.dropQps.length != 0) {
            var qps = getData(obj.qps, 2);
            var dropQps = getData(obj.dropQps, 2);

            var dropQpsSum = dropQps.reduce(function (a, b) {
                return parseFloat(a) + parseFloat(b);
            }, 0);
            if (dropQpsSum != 0) {
                var result = [];
                for (var i = 0, l = qps.length; i < l; i++) {
                    result.push((qps[i] - dropQps[i]).toFixed(2));
                }
                ...
                legend.data.push({name: 'QPS', icon: 'line'});
    ...
    function tableTemplate() {
        ...
        return '<table id="kpi_table" class="table table-striped table-hover " data-widget="sortTable"> ' +
            ...
            '<tr>' +
            '<td style="width: 18%">' +
            '<a href="/service/detail?appkey=' + appkey + '&env=' + env + '&range=1h&remoteApp=<%= item.tags.remoteApp %>&remoteHost=*#consumer" target="_blank"><%= item.tags.remoteApp %></a>' +
            '<% if (item.tags.remoteApp == "unknownService") {%>' +
            '<a href="https://123.sankuai.com/km/page/28354817" target="_blank"><i class="fa fa-question-circle"></i></a>' +
            '<% } %>' +
            '</td>' +
    ...
    var hostDetailTemplate =
        ...
        '<thead>' +
        '<tr>' +
        '<th style="text-overflow:ellipsis; overflow: hidden;white-space: nowrap; background-color: #ffffff;">来源IP(客户端)</th>' +
