M.add('msgp-dashboard/database', function (Y) {
        Y.namespace('msgp.dashboard').database = database;
        var startInput = Y.one('#start_time'),
            endInput = Y.one('#end_time');
        var detailDialog;
        var nodeSearch = "corp%3Dmeituan%26owt%3Ddba",
            _alarmDate = {};

        function database() {
            document.title = '服务治理平台-数据库大盘';
            initTimeLine();
            initDatePicker();
            getData();
        ...
        function bindDetailAlarm() {
            Y.one('#falcon_alarm').delegate('click', function () {
        ...
        function initTimeLine(){
            Y.all("#timeline a").on('click',function(e){
                ...
                window.location.href="/database?start="+start_time+"&end="+end_time
            })
        }
...
M.use('msgp-dashboard/database', function (Y) {
    Y.msgp.dashboard.database();
});
