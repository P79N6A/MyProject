...
for env in ${all_env[@]} 
do
agent_bin_dir=${env}"_agent_bin_online"
if [ ! -x "$agent_bin_dir" ]; then
    mkdir "$agent_bin_dir"
fi

agent_bin_worker_dir=${env}"_worker_bin_online"
...
cp -f build/bin/sg_agent_mutable_online_${env}.xml $agent_bin_dir/sg_agent_mutable.xml
cp -f build/bin/sg_agent_online_servicelist.xml $agent_bin_dir/sg_agent_servicelist.xml
...
cp -f build/bin/sg_agent_mutable_online_${env}.xml $agent_bin_worker_dir/sg_agent_mutable.xml
cp -f build/bin/sg_agent_online_servicelist.xml $agent_bin_worker_dir/sg_agent_servicelist.xml
done
