...
object OctoDataCenterDay {

  private val role2TableNameMap = Map(0 -> "octolog_cli_perf_day_index", 1 -> "octolog_serv_perf_day_index")
  ...
  def main(args: Array[String]) {
    ...
    dayTuples.foreach { dayTuple =>
      ...
      val rawInfos = dataFrame.map { row =>
        ...
        val dayTs = Helper.getDayStart(start)
        ...
        (new ModuleInvokeAllKey(localAppKey, dayTs, _type, envStr, spanName, localhost, remotAppKey, remoteHost),
          Map[Int, Long](cost -> count.toLong))
      }
      val infos = rawInfos.reduceByKey(Helper.mergeCostToCountMap).persist(StorageLevel.MEMORY_AND_DISK_SER)
      ...
      val spanAllHive = spanAllRdd.flatMap { case (dayKey, resultMap) =>
        resultMap.toSeq.flatMap { case (tuple, costToCount) =>
          val tags = getTags(spanname = tuple)
          val statDataList = Calculator.calculate(dayKey, Seq(StatGroup.Span, StatGroup.SpanRemoteApp,
            StatGroup.SpanLocalHost, StatGroup.SpanRemoteHost, StatGroup.LocalHostRemoteApp),
            tags, costToCount)(StatRange.Day)
          statDataList.map { statData =>
            transformStatDataToHiveData(statData)
          }
        }
      }
      val spanHive = spanRdd.flatMap { case (dayKey, resultMap) =>
        resultMap.toSeq.flatMap { case (tuple, costToCount) =>
          val tags = getTags(spanname = tuple)
          val statDataList = Calculator.calculate(dayKey, Seq(StatGroup.Span, StatGroup.SpanRemoteApp,
            StatGroup.SpanLocalHost, StatGroup.SpanRemoteHost), tags, costToCount)(StatRange.Day)
          statDataList.map { statData =>
            transformStatDataToHiveData(statData)
          }
        }
      ...
      val localhostHive = localhostRdd.flatMap { case (dayKey, resultMap) =>
        resultMap.toSeq.flatMap { case (tuple, costToCount) =>
          val tags = getTags(localHost = tuple)
          val statDataList = Calculator.calculate(dayKey, Seq(StatGroup.SpanLocalHost, StatGroup.LocalHostRemoteApp),
            tags, costToCount)(StatRange.Day)
          statDataList.map { statData =>
            transformStatDataToHiveData(statData)
          }
      ...
      val remoteAppHive = remoteAppRdd.flatMap { case (dayKey, resultMap) =>
        resultMap.toSeq.flatMap { case (tuple, costToCount) =>
          val tags = getTags(remoteAppKey = tuple)
          val statDataList = Calculator.calculate(dayKey, Seq(StatGroup.SpanRemoteApp, StatGroup.LocalHostRemoteApp),
            tags, costToCount)(StatRange.Day)
          statDataList.map { statData =>
            transformStatDataToHiveData(statData)
          }
      ...
      val remoteHostHive = remoteHostRdd.flatMap { case (dayKey, resultMap) =>
        resultMap.toSeq.flatMap { case (tuple, costToCount) =>
          val tags = getTags(remoteHost = tuple)
          val statDataList = Calculator.calculate(dayKey, Seq(StatGroup.SpanRemoteHost), tags,
            costToCount)(StatRange.Day)
          statDataList.map { statData =>
            transformStatDataToHiveData(statData)
          }
        }
      }
      val spanRemoteAppHive = rddToHiveData(spanRemoteAppRdd, StatGroup.SpanRemoteApp)(StatRange.Day)
      val spanLocalHostHive = rddToHiveData(spanLocalHostRdd, StatGroup.SpanLocalHost)(StatRange.Day)
      val spanRemoteHostHive = rddToHiveData(spanRemoteHostRdd, StatGroup.SpanRemoteHost)(StatRange.Day)
      val localHostRemoteAppHive = rddToHiveData(localHostRemoteAppRdd, StatGroup.LocalHostRemoteApp)(StatRange.Day)
