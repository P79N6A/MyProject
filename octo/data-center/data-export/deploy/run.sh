...
JVM_HEAP="-XX:+HeapDumpOnOutOfMemoryError -XX:+UseStringDeduplication -XX:+PrintStringDeduplicationStatistics -XX:+PrintStringTableStatistics"
JVM_SIZE="-Xmx14g -Xms14g -Xmn6g -XX:SurvivorRatio=4 -XX:MaxTenuringThreshold=15"
...
# ------------------------------------
MODULE=data-export
WORK_PATH=/opt/meituan/mobile/data-export
LOG_PATH=/opt/logs/mobile/data-export
WEB_ROOT=$WORK_PATH/webroot

function init() {
    preRun
    #   提取出octo.keytab用于hbase登录
    unzip -o data-export-release-0.2.0.jar octo.keytab
    mkdir -p ${LOG_PATH}
}
...
function run() {
    ...
    ${EXEC_JAVA} -jar data-export-release-0.2.0.jar > /tmp/$MODULE.start.log 2>&1
}
