...
class AsyncProcessor[T] private(private val workerCount: Int = 5, private val processor: (T => Unit), private val name: String = "async") {
  val executor = Executors.newFixedThreadPool(workerCount, StatThreadFactory.threadFactory(name))
...
object AsyncProcessor {
  ...
  def apply[T](workerCount: Int, processor: (T => Unit), name: String) = {
    val res = new AsyncProcessor[T](workerCount, processor, name)
    res.init()
    res
  }
}
