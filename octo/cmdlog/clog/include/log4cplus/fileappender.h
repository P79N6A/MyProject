...
namespace log4cplus
{
    ...
    enum CmdLogAppendType { LOCAL_DISK = 1, LOG_AGENT = 2, BOTH_TYPE = 3}; 
    ...
    public:
      // Ctors
        DailyRollingFileAppender(const log4cplus::tstring& filename,
                                 ...
                                 int maxHistory = 10,
                                 std::string app_key = "default_app_key" ,
                                 int appendType = 1,
    ...
    protected:
        virtual void append(const spi::InternalLoggingEvent& event);
        void appendToDisk(const spi::InternalLoggingEvent& event); 
        void appendToLogAgent(const spi::InternalLoggingEvent& event);
        void rollover(bool alreadyLocked = false, bool rolloverByTime = false);
      ...
      // Data
        ...
        bool appendDisk;
        bool appendLogAgent; 
        cmdlog::scribe::ScribeClientPtr scribe_client;
        cmdlog::scribe::ScribeClientConfig scribe_conf;

    private:
        LOG4CPLUS_PRIVATE void init(DailyRollingFileSchedule schedule,long maxFileSize,std::string appKey = "default_app_key",int appendType_ = 1);
    };
...
class LOG4CPLUS_EXPORT ScribeAppender : public FileAppender {
 public:
    ScribeAppender(std::string app_key,int rate_limit = 0);
	virtual ~ScribeAppender();
	virtual void close();
    static void clearAppendRecord();
protected:
    bool hasReachRateLimit();
    void* setTimer(void* arg);
	virtual void append(const spi::InternalLoggingEvent& event);
    cmdlog::scribe::ScribeClientPtr scribe_client;
    cmdlog::scribe::ScribeClientConfig scribe_conf;
private:
	LOG4CPLUS_PRIVATE void init(std::string appKey = "default_app_key", int rate_limit = 0);
    int appendLimitPerSecond; // how many logs send to log agent per second
    static int hasAppendLogsPerSecond;
};							

typedef helpers::SharedObjectPtr<ScribeAppender>
        SharedScribeAppenderPtr;
