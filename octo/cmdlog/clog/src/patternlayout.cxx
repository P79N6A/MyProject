...
public:
    ...
    void formatAndAppend(std::string& content,
        const spi::InternalLoggingEvent& event);
...
void
PatternConverter::formatAndAppend(
    std::string& content, const spi::InternalLoggingEvent& event)
{
    tstring & s = internal::get_ptd ()->faa_str;
    convert (s, event);
    std::size_t len = s.length();
...
void
PatternLayout::formatAndAppendForScribe(std::string& content,
                               const spi::InternalLoggingEvent& event)
{
    content = "";
    for(PatternConverterList::iterator it=parsedPattern.begin();
        ...
        ++it)
    {
        (*it)->formatAndAppend(content,event);
    }
}
