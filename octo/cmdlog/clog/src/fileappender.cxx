...
DailyRollingFileAppender::DailyRollingFileAppender(
    ...
    bool immediateFlush_, long maxFileSize_, int maxBackupIndex_, int maxHistory_, std::string app_key, int appendType_, bool cleanHistoryOnStart_, 
    bool createDirs_, bool rollOnClose_, const tstring& datePattern_)
    : FileAppender(filename_, std::ios_base::app, immediateFlush_, createDirs_)
...
{
    init(schedule_,maxFileSize_,app_key,appendType_);
}
...
void
DailyRollingFileAppender::init(DailyRollingFileSchedule sch,long maxFileSize_,std::string appKey_,int appendType_)
{
    this->schedule = sch;
    ...
    if (appendLogAgent){ //only scribe is on
        scribe_conf.host = "127.0.0.1";
        scribe_conf.port = 4252;
        scribe_conf.category = appKey_;
        try{
            scribe_client = boost::shared_ptr<cmdlog::scribe::ScribeClient>(new cmdlog::scribe::ScribeClient(scribe_conf));
        }catch(std::exception& e){
			TBSYS_LOG(ERROR,"!!!###catch exception %s",e.what());
			try{
			scribe_conf.host = "log_agent.test.data.sankuai.com";
            scribe_client = boost::shared_ptr<cmdlog::scribe::ScribeClient>(new cmdlog::scribe::ScribeClient(scribe_conf));
			}catch(std::exception& e){
			TBSYS_LOG(ERROR,"!!!###catch exception %s",e.what());
			}
...
void
DailyRollingFileAppender::appendToLogAgent(const spi::InternalLoggingEvent& event)
{
    std::string content = "";
    layout->formatAndAppendForScribe(content, event);
    try{
        scribe_client->send(content);
    }catch(std::exception& e){
        TBSYS_LOG(ERROR, "catch exception: %s",e.what()); 
    }
...
void
DailyRollingFileAppender::appendToDisk(const spi::InternalLoggingEvent& event)
{
    if (useLockFile)
        out.seekp (0, std::ios_base::end);
    if (out.tellp() > maxFileSize) {
...
void
DailyRollingFileAppender::rollover(bool alreadyLocked, bool rolloverByTime)
{
    helpers::LockFileGuard guard;
    ...
    //if rolling is triggered by time
    //calc next rollover time
    //clean expired output files
...
//add by sun;
//funtion: delete at regular time
void
DailyRollingFileAppender::clean(Time time)
{
    Time interval;
    switch (schedule)
    {
    case MONTHLY:
        interval = Time(12*31*24*3600); // ~1 year
        break;
    case WEEKLY:
        interval = Time(10*7*24*3600); // ~10 weeks 
        break;
    case DAILY: 
        interval = Time(31*24*3600); // ~1 month
        break;
    case HOURLY:
        interval = Time(24*3600); // ~1 day
        break;
    case MINUTELY:
        interval = Time(3600); // ~1 hour
        break;
    default:
        helpers::getLogLog ().error (
        LOG4CPLUS_TEXT ("DailyRollingFileAppender::clean()-")
    ...
    for (long i = 0; i < periods; i++)
    {
        ...
        tstring filenameToRemove = getFilename(timeToRemove);
        loglog.debug(LOG4CPLUS_TEXT("Removing file ") + filenameToRemove);
        long ret = file_remove(filenameToRemove);
        // delete filename.1 filename.2 ...
        tostringstream buffer1;
        for (int j = 1; j <= maxBackupIndex; ++j){
            buffer1.str(LOG4CPLUS_TEXT("")); 
            buffer1 << filenameToRemove << LOG4CPLUS_TEXT(".") << j;
            tstring const fileToRemove(buffer1.str ());
            ret = file_remove (fileToRemove);
        }
    }
    
    if (periods > 0) //improtant: not update lastHeartBeat when rolling over in sepcial size  
        lastHeartBeat = time;
}
...
void proc(int){
    ScribeAppender::clearAppendRecord();    
}

int ScribeAppender::hasAppendLogsPerSecond = 0;

ScribeAppender::ScribeAppender(
    std::string app_key,int rate_limit)
    : FileAppender(LOG4CPLUS_TEXT ("no"), std::ios_base::app, false, false)
{
    init(app_key,rate_limit);
    setTimer(NULL);
}

void
ScribeAppender::init(std::string appKey_,int rate_limit)
{
	cmdlog::DevInfo devinfo;
	bool isOnline = devinfo.isOnlineDev();
	TBSYS_LOG(DEBUG,"dev env is online? 0 == offline 1 == online: %d",isOnline);
    //无论线上还是线下第一次都尝试发送本地的log agent
    scribe_conf.host = "127.0.0.1";
	scribe_conf.port = 4252;
	scribe_conf.category = appKey_;
	scribe_conf.timeout_ms = 200;
	scribe_conf.send_interval_sec = 1;
	try{
		TBSYS_LOG(DEBUG,"send to localhost log_agent first");
		scribe_client = boost::shared_ptr<cmdlog::scribe::ScribeClient>(new cmdlog::scribe::ScribeClient(scribe_conf));
	} catch (std::exception& e){
    //如果本地log agent不在,区分环境
    //线上环境仍然连接本地，如果还不存在，等待200ms,如果仍然没有，crash
    //线下环境发送test域名
	    scribe_conf.host = isOnline == true ? "127.0.0.1":"log_agent.test.data.sankuai.com";
        if (isOnline)
        {
		    TBSYS_LOG(ERROR,"!!!###create scribe_client--->catch exception %s",e.what());
        }
	    TBSYS_LOG(DEBUG,"log agent addr is : %s",scribe_conf.host.c_str());
		try{
		    scribe_client = boost::shared_ptr<cmdlog::scribe::ScribeClient>(new cmdlog::scribe::ScribeClient(scribe_conf));
		} catch (std::exception& e){
		    TBSYS_LOG(ERROR,"!!!###create scribe_client--->catch exception %s",e.what());
            if (CLOG_LIKELY(isOnline == true)){
                TBSYS_LOG(DEBUG,"wait for 200ms for log agent start");
                usleep(200000);
		        scribe_client = boost::shared_ptr<cmdlog::scribe::ScribeClient>(new cmdlog::scribe::ScribeClient(scribe_conf));
            } else {
                TBSYS_LOG(DEBUG,"wait for 1ms,retry send to log_agent.test.data again ");
                usleep(1000);
		        scribe_client = boost::shared_ptr<cmdlog::scribe::ScribeClient>(new cmdlog::scribe::ScribeClient(scribe_conf));
            }
		}
    ...
    appendLimitPerSecond = rate_limit;
}
...
void
ScribeAppender::clearAppendRecord(){
    hasAppendLogsPerSecond = 0;
}


void* 
ScribeAppender::setTimer(void* arg){
    signal(SIGALRM,proc);
    itimerval timerval;
...
bool
ScribeAppender::hasReachRateLimit()
{
    return (appendLimitPerSecond !=0) && (hasAppendLogsPerSecond >= appendLimitPerSecond);    
}

void
ScribeAppender::append(const spi::InternalLoggingEvent& event)
{
    if (hasReachRateLimit()) return;
    std::string content = "";
    layout->formatAndAppendForScribe(content, event);
    try{
        int retry = 0;
        bool res;
        while(retry < 3){
            res = scribe_client->syncsend(content);
            if (res) {
                if (appendLimitPerSecond != 0) hasAppendLogsPerSecond++;
                break;
            }
            retry ++;
            usleep(retry*10);
        }
        TBSYS_LOG(DEBUG, "send content %s",content.c_str()); 
    }catch(std::exception& e){
        TBSYS_LOG(ERROR, "sync send log --- >catch exception: %s log is : %s",e.what(),content.c_str()); 
        try{
            usleep(20);
            scribe_client->syncsend(content);
        }catch(std::exception& e){
                TBSYS_LOG(ERROR, "sync send log failed again after sleep--- >catch exception: %s log is : %s",e.what(),content.c_str()); 
            }
